...
contract UniversalPaymasterTest is Test, UserOpHelper {
    ...
    function test_sponsor_user_operation() public {
        ...
        assertEq(token.allowance(address(accountECDSA), address(paymaster)), type(uint256).max);
        ...
        bytes memory callData = abi.encodeCall(
            ...
            (address(token), 0, abi.encodeWithSelector(token.transfer.selector, receiver, 1e18))
        );
        ...
        console.log("Smart account code size:", address(accountECDSA).code.length);
        ...
        assertEq(token.balanceOf(receiver), 1e18, "Receiver should have received 1 token");
